...
namespace Redcode.Pools
{
    ...
    public class Pool<T> : IPool<T> where T : Component
    {
        ...
        #region SetSource(added)

        IPool IPool.SetSource(Component newSource, bool deactivateClones) => SetSource(newSource, deactivateClones);
        IPool<T> IPool<T>.SetSource(Component newSource, bool deactivateClones) => SetSource(newSource, deactivateClones);

        public Pool<T> SetSource(Component newSource, bool deactivateClones = false)
        {
            if (deactivateClones)
            {
        ...
        public Pool<T> SetCount(int count, bool destroyClones = true)
        {
            ...
            // Error occurs when _clones.Count < count
            //_clones.RemoveRange(count, _clones.Count - count);

            #endregion

            #region Revised Code (Added condition for RemoveRange)
        ...
        Component IPool.Get(Vector3 pos) => Get(pos);
        /// <summary>
